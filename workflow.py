...
# Edited by Laura Kragh Frederiksen 11/06/2023
# ----------------------------------------------------------------------------------------------------------------------------
# ----------------------------------------------------------------------------------------------------------------------------
# This workflow includes 3 separate workflows: 
# 1. Workflow to reconstruct the 'Final species tree', using supercontings in the ingroup and a combination of exons and aupercontigs in the outgroup to reconstruct the PAFTOL topology.
# 2. Workflow to determine how to reconstruct the outgroup topology using a combination of supercontigs and exons.
# 3. Workflow to select genes for molecular dating in BEAST.
...
# ### Workflow 2
# ----------------------------------------------------------------------------------------------------------------------------
# 1. IQ-tree (exon) - getting partitioner exon gene trees to match the supercontig gene trees. 
# 2. Rooting gene trees
# 3. SortaDate - finding the exons that are most like the species tree
# 4. Folder - combine different combinations oft supercontigs and exons until the required outgroup topology has been achieved.
# 5. Generate ASTRAL tree from these gene trees. 

# ----------------------------------------------------------------------------------------------------------------------------
# ### Workflow 3 ###
# ----------------------------------------------------------------------------------------------------------------------------
# FYI - these are just the steps that required a cluster - see GitHub
# 1. Rooting gene trees
# 2. SortaDate - finding the genes with least conflict compared with the species tree. 
# 3. IQtree modelfinder - find best substitution model for these genes.
# 4. Run BEAST
...
#######################################################################################################################################
# FYI: We also tried trimming the raw reads using MAXINFO, but this method did not provide as good trimming results.
...
################################################---- Move files ----###############################################################
###################################################################################################################################
# At this step all the 'PAFTOL' outgroup samples were moved to the folder and
# The remaining samples 'ingroup' were moved to the folder
# This was done to make the 'coverage' step easier.

# I have not included this part in the workflow, but this can be done with a simple 'mv' command of the PAFTOL samples from the /Workflow/Final_tree/03_Trimmomatic/slidingwindow/ folder a newly generated folder /Workflow/Final_tree/03_Trimmomatic/slidingwindow/Outgroup/.
# Likewise the AL* samples from the /Workflow/Final_tree/03_Trimmomatic/slidingwindow/ folder to the newly generated /Workflow/Final_tree/03_Trimmomatic/slidingwindow/.

# eg.e. from within /Workflow/Final_tree/03_Trimmomatic/slidingwindow/:
# mv *PAFTOL /Workflow/Final_tree/03_Trimmomatic/slidingwindow/Outgroup/

# From the abovementioned folders HybPiper retrieved the trimmed sequences. 
...
def hybpiper_outgroup(name, p1, p2, un, path_out, path_in, done):
    ...
    # Here I have used the Rohwer target file, which also includes some plastid genes (these are not used in our further analysis)
...
# At this step you can blacklist species if you dont want them in the downstream analysis
# I have added the specimens Synd-chine, Peum-boldu and Chim-salic as gene files from PAFTOL before retrieving. The files were altered to match the format of the remaining coverage files.
...
# Run the script optrimal.R in interactive mode from the folder 10_trimal where the summary files generated in the two AMAS scripts are, if the R environment does not work.
...
####################################################################################################################################################
##############################################---- Move optrimal result files to new folder ----###########################################################
####################################################################################################################################################

# # #### Move files from trimal to optrimal folder (only necessary if R script will not run within the cluster).
...
# def hyb_move(path_in,path_out,done):
...
#     #Going to folder with data (HybPiper outgroup)
#     cd {path_in}
...
def astral2(path_in, path_out, done):
    ...
    java -jar /home/lfrederi/scratch/apps/ASTRAL/Astral/astral.5.7.8.jar -i Beilschmiedia-Outgroup_trees_BP10.tre -t 2 -o Beilschmiedia-Outgroup_trees_BP10_SpeciesTree_annotQ.tre

    java -jar /home/lfrederi/scratch/apps/ASTRAL/Astral/astral.5.7.8.jar -i Beilschmiedia-Outgroup_trees_BP10.tre -t 0 -o Beilschmiedia-Outgroup_trees_BP10_SpeciesTree.tre
...
# # First manually copy supercontig partition files to path_out
...
#### Here I run ASTRAL to get a species tree based on the combination of gene trees above. 
...
# # ##############################################--- Molecular clock dating with BEAST ---#################################################
...
## Running IQtrees modelfinder to identify the 'best' genes for dating. 

# def iq_tree_modelfinder(path_in, gene_BEAST, path_out, done):
#     """Using Iq-tree modelfinder to find the best substitution model for each gene without partitioning"""
#     inputs = [path_in+gene_BEAST+"_output_taper.fasta"]
#     outputs = [done]
#     options = {'cores': 2, 'memory': "2g", 'walltime': "00:30:00", 'account':"cryptocarya"}
...
#     cp {gene_BEAST}_output_taper.fasta {path_out}
...
#     """.format(path_in = path_in, gene_BEAST = gene_BEAST, path_out = path_out, done = done)
...
# Removing files that are empty or not present: 5163, 5427, 5599, 5614, 5921, 5943, 6041
# Removing genes that were not included in "optimal_final_results", - 5943, 5163, 5427, 5599, 5614

# gene = ["4471", "4527", "4691", "4724", "4744", "4757", "4793", "4796", "4802", "4806", "4848", "4889", "4890", "4893", "4932", "4942", "4951", "4954", "4989", "4992", "5018", "5032", "5034", "5038", "5064", "5090", "5104", "5116", "5123", "5131", "5138", "5162", "5168", "5177", "5188", "5200", "5206", "5220", "5257", "5260", "5264", "5271", "5273", "5280", "5296", "5299", "5304", "5318", "5326", "5328", "5333", "5335", "5339", "5343", "5347", "5348", "5354", "5355", "5357", "5366", "5398", "5404", "5406", "5421", "5426", "5428", "5430", "5434", "5449", "5454", "5460", "5463", "5464", "5469", "5477", "5489", "5502", "5513", "5528", "5531", "5536", "5551", "5554", "5562", "5578", "5594", "5596", "5620", "5634", "5639", "5642", "5644", "5656", "5660", "5664", "5670", "5699", "5702", "5703", "5716", "5721", "5733", "5744", "5770", "5772", "5791", "5802", "5815", "5816", "5821", "5822", "5840", "5841", "5842", "5843", "5849", "5853", "5857", "5858", "5859", "5865", "5866", "5870", "5893", "5894", "5899", "5910", "5913", "5918", "5919", "5922", "5926", "5933", "5936", "5940", "5941", "5942", "5944", "5945", "5949", "5950", "5958", "5960", "5968", "5974", "5977", "5980", "5981", "5990", "6000", "6003", "6004", "6016", "6026", "6029", "6034", "6036", "6038", "6048", "6050", "6051", "6056", "6064", "6068", "6072", "6098", "6110", "6114", "6119", "6128", "6130", "6139", "6148", "6150", "6164", "6175", "6176", "6198", "6216", "6221", "6226", "6227", "6238", "6258", "6265", "6270", "6274", "6282", "6284", "6295", "6298", "6299", "6303", "6318", "6320", "6363", "6366", "6373", "6376", "6378", "6379", "6383", "6384", "6387", "6389", "6393", "6398", "6404", "6405", "6406", "6407", "6412", "6420", "6430", "6432", "6439", "6447", "6448", "6449", "6450", "6454", "6457", "6458", "6459", "6460", "6462", "6483", "6487", "6488", "6492", "6494", "6496", "6498", "6500", "6506", "6507", "6526", "6527", "6528", "6532", "6533", "6538", "6540", "6544", "6550", "6552", "6557", "6559", "6563", "6565", "6570", "6572", "6601", "6620", "6631", "6636", "6639", "6641", "6649", "6652", "6660", "6667", "6679", "6685", "6689", "6705", "6713", "6717", "6732", "6733", "6738", "6746", "6779", "6780", "6782", "6785", "6791", "6792", "6797", "6825", "6848", "6854", "6859", "6860", "6864", "6865", "6875", "6882", "6883", "6886", "6893", "6909", "6913", "6914", "6924", "6933", "6946", "6947", "6954", "6955", "6958", "6961", "6962", "6968", "6969", "6977", "6978", "6979", "6992", "6995", "7013", "7021", "7024", "7028", "7029", "7067", "7111", "7128", "7135", "7136", "7141", "7174", "7194", "7241", "7273", "7279", "7296", "7313", "7324", "7325", "7331", "7333", "7336", "7361", "7363", "7367", "7371", "7572", "7577", "7583", "7602", "7628"]
# # Running CIAlign on the trimmed_fasta - excluded some genes earlier
# for i in range(0, len(gene)):
    # gwf.target_from_template('Cialign_'+gene[i], cialign1(gene = gene[i],
...
## #### Move files from HybPiper outgroup to HybPiper ingroup folder
## gwf.target_from_template('hyb_move', hyb_move(path_in = "/home/laurakf/cryptocarya/Workflow/Final_tree/06_HybPiper/Outgroup/",
##                                                 done = "/home/laurakf/cryptocarya/Workflow/Final_tree/06_HybPiper/hyb_move", 
##                                                 path_out = "/home/laurakf/cryptocarya/Workflow/Final_tree/06_HybPiper/Ingroup/"))
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------
#  WORKFLOW 2: PAFTOL outgroup   ### This part is a bit incomplete. 
# ------------------------------------------------------------------------------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------------------------------------------------------------------------------
#  WORKFLOW 3: Molecular clock analysis with BEAST
# ------------------------------------------------------------------------------------------------------------------------------------------------------
...
# --------------------------------------
# Before these steps I manually cleaned the alignments I wanted to use in the final analyses analysis.
# I generated the input file using the GUI BEAUti

### IQtree ###
### Ended up just using the previously generated report of substitution models for the different genes

### BEAST ###
### Ended up running BEAST using a normal script, as I had BEAST issues due to some JAVA problem. 
